...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise YTDLError('Nie mogłem znaleść pasujących wyników dla `{}`'.format(search))
        ...
        if processed_info is None:
            raise YTDLError('Nie udało się pobrać `{}`'.format(webpage_url))
        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise YTDLError('Nie można pobrać żadnych dopasowań dla `{}`'.format(webpage_url))
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Teraz gra',
                 ...
                 .add_field(name='Uruchomił', value=self.requester.mention)
                 .add_field(name='Przesyłał', value='[{0.source.uploader}]({0.source.uploader_url})'.format(self))
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('To polecenie nie może być używane w kanałach DM.')
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send('Wystąpił błąd : {}'.format(str(error)))
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('Nie jesteś połączony z kanałem głosowym ani nie określiłeś kanału, do którego chcesz dołączyć.')
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if 0 > volume > 100:
            return await ctx.send('Głośność musi wynosić od 0 do 100')

        ctx.voice_state.volume = volume / 100
        await ctx.send('Głośność odtwarzacza ustawiona na {}%'.format(volume))
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('W tej chwili nie odtwarzam żadnej muzyki...')
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Dodano głosowanie na pominięcie, obecnie: **{}/3**'.format(total_votes))

        else:
            await ctx.send('Już zagłosowałeś, aby pominąć tę piosenkę.')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send('Wystąpił błąd podczas przetwarzania tego żądania: {}'.format(str(e)))
            else:
                song = Song(source)
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Nie masz połączenia z żadnym kanałem głosowym.')

        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError('Bot jest już w kanale głosowym.')
